...
vi.mock('@/lib/purchase-receipts', async () => {
  ...
  return {
    ...actual,
    getPurchaseReceipt: receiptMocks.get,
    listReceiptStorageLocations: receiptMocks.locations,
  };
});
...
const PO = {
  ...
  farm_id: null,
  lines: [{ id: 'line-1', line_number: 1, item_name: 'Feed' }],
} as PurchaseOrder;
const RECEIPT = {
  id: 'r-1',
  organization_id: 'org-1',
  ...
  warehouse_id: 'wh-1',
  grn: 'GRN-2026-000001',
  supplier_delivery_reference: 'DEL-1',
...
const props = {
  purchaseOrder: PO,
  warehouseLabels: new Map([['wh-1', 'Main (MAIN)']]),
...
describe('PurchaseReceiptHistory', () => {
  beforeEach(() => {
    receiptMocks.get.mockResolvedValue(RECEIPT);
    receiptMocks.locations.mockResolvedValue([]);
  });
  it('renders its empty state', () => {
    render(<PurchaseReceiptHistory {...props} receipts={[]} />);
  ...
  it('loads immutable receipt detail with exact quantities', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('receipt-detail')).toHaveTextContent('0.000001 kg'),
    );
  ...
  it('uses cursor navigation callbacks', () => {
    const next = vi.fn();
    render(
      <PurchaseReceiptHistory {...props} receipts={[RECEIPT]} nextCursor="opaque" onNext={next} />,
    );
  ...
  it('keeps a hidden 404 generic', async () => {
    ...
    receiptMocks.get.mockRejectedValue(
      new ApiError(404, {
        detail: { code: 'not_found', message: 'foreign receipt exists' },
      } as never),
    );
    render(<PurchaseReceiptHistory {...props} receipts={[RECEIPT]} />);
    fireEvent.click(screen.getByRole('button', { name: /GRN-2026-000001/i }));
