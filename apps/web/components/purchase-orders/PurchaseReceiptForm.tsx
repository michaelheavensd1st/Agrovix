...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  canonical_quantity_not_representable:
    'This quantity cannot be represented exactly in the item’s canonical unit.',
  purchase_order_over_receipt:
    'The receipt exceeds the remaining Purchase Order quantity. Refresh and review the line.',
  warehouse_unavailable: 'The selected warehouse is no longer available.',
  warehouse_farm_scope_mismatch:
    'The selected warehouse does not match this Purchase Order’s farm.',
  ...
  lot_attribute_conflict:
    'An existing lot with this code has different location or expiry details.',
...
export function PurchaseReceiptForm({
  ...
  useEffect(() => {
    ...
    void listReceiptWarehouses(purchaseOrder.id, controller.signal)
      .then((items) => {
        if (
          !mountedRef.current ||
          generation !== warehouseGenerationRef.current ||
          capturedContext !== currentContextRef.current
        )
          return;
        setWarehouses(
          items.filter(
    ...
    void listReceiptStorageLocations(warehouseId, controller.signal)
      .then((items) => {
        if (
          ...
          capturedWarehouseId !== currentWarehouseIdRef.current ||
          capturedContext !== currentContextRef.current
        )
          return;
      ...
      .catch((caught) => {
        ...
        setLocationError(
          caught instanceof ApiError && caught.status === 403
            ? 'Storage locations are unavailable with your current permissions.'
            : 'Unable to load storage locations.',
        );
      });
    return () => controller.abort();
  }, [open, warehouseId]);
  ...
  function changeWarehouse(nextWarehouseId: string) {
    ...
    setLines((current) =>
      Object.fromEntries(
        Object.entries(current).map(([id, value]) => [id, { ...value, storageLocationId: '' }]),
      ),
    );
  }
  ...
  function buildPayload(): PurchaseReceiptInput | null {
    ...
    if (supplierReference.trim().length > 120)
      nextErrors.supplierReference = 'Use 120 characters or fewer.';
    ...
    const payloadLines = selected.flatMap((line) => {
      ...
      const remaining = subtractPurchaseOrderDecimals(
      ...
      if (
        ...
        nextErrors[`${line.id}.quantity`] =
          'Enter a positive quantity with at most 18 digits and six decimal places.';
      else if (comparePurchaseOrderDecimals(value.quantity, remaining) > 0)
        nextErrors[`${line.id}.quantity`] = 'Quantity is above the currently remaining amount.';
      if (!value.lotCode.trim()) nextErrors[`${line.id}.lot`] = 'Enter a lot code.';
      else if (value.lotCode.trim().length > 128)
        nextErrors[`${line.id}.lot`] = 'Use 128 characters or fewer.';
      if (nextErrors[`${line.id}.quantity`] || nextErrors[`${line.id}.lot`]) return [];
      return [
        {
          purchase_order_line_id: line.id,
          lot_code: value.lotCode.trim(),
          quantity: canonicalPurchaseOrderDecimal(value.quantity),
          ...(value.storageLocationId ? { storage_location_id: value.storageLocationId } : {}),
          ...(value.expiryDate ? { expiry_date: value.expiryDate } : {}),
        },
      ];
    });
    ...
    if (receivedAt) {
      ...
      if (Number.isNaN(date.getTime())) {
        setErrors({ receivedAt: 'Enter a valid date and time.' });
        return null;
      }
    ...
    return {
      ...
      supplier_delivery_reference: nullable(supplierReference),
    ...
    try {
      ...
      if (
        ...
        if (caught.status === 401) {
          ...
          setAttempt(null);
          setGeneralError(
            'This submission could not be replayed because its saved data differs. Review the form before starting a new submission.',
          );
        } else {
          setAttempt(null);
          if (code === 'purchase_order_over_receipt' || code === 'purchase_order_not_receivable')
            onAuthoritativeFailure('purchase-order-changed');
          ...
          if (code === 'not_authorized' || caught.status === 403)
            onAuthoritativeFailure('authorization-changed');
          if (code === 'invalid_quantity') {
            const quantityErrors = Object.fromEntries(
              purchaseOrder.lines
                .filter((line) => lines[line.id]?.included)
                .map((line) => [`${line.id}.quantity`, ERROR_MESSAGES.invalid_quantity]),
            );
            ...
            for (const entry of detail) {
              ...
              const lineIndex =
                loc[0] === 'body' && loc[1] === 'lines' && typeof loc[2] === 'number'
                  ? loc[2]
                  : null;
              const field = loc[3];
              const selected = purchaseOrder.lines.filter((line) => lines[line.id]?.included);
              const poLine = lineIndex === null ? null : selected[lineIndex];
              if (poLine && field === 'quantity')
                mapped[`${poLine.id}.quantity`] = 'Review this receipt quantity.';
              if (poLine && field === 'lot_code')
                mapped[`${poLine.id}.lot`] = 'Review this lot code.';
            }
            setErrors(mapped);
          }
          const message =
            code === 'idempotency_key_required'
              ...
              : typeof code === 'string'
                ? (ERROR_MESSAGES[code] ??
                  'Unable to post this receipt. Review the form and try again.')
                : 'Unable to post this receipt. Review the form and try again.';
  ...
  function retry(current: Attempt) {
    if (submitLockRef.current) return;
    submitLockRef.current = true;
    void send(current);
  }
  ...
  return (
    <div
      ...
      data-testid="receive-po-dialog"
    >
      <div
        ref={dialogRef}
        className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-border bg-card p-5 shadow-lg"
      >
        <h2 id="receive-po-title" ref={titleRef} tabIndex={-1} className="font-display text-xl">
          Receive {purchaseOrder.po_number}
        </h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Enter quantities in each Purchase Order line’s ordered unit. Posting is immutable.
        </p>
        {generalError && (
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <Field label="Warehouse" error={errors.warehouse} errorId="receipt-error-warehouse">
            <select
              aria-invalid={Boolean(errors.warehouse)}
              aria-describedby={errors.warehouse ? 'receipt-error-warehouse' : undefined}
              value={warehouseId}
              disabled={frozen || busy || Boolean(optionsError)}
          ...
          <Field
            ...
            errorId="receipt-error-supplier-reference"
          >
            <input
              aria-invalid={Boolean(errors.supplierReference)}
              aria-describedby={
                errors.supplierReference ? 'receipt-error-supplier-reference' : undefined
              }
          ...
          <Field
            ...
            errorId="receipt-error-received-at"
          >
            <input
              aria-invalid={Boolean(errors.receivedAt)}
              aria-describedby={errors.receivedAt ? 'receipt-error-received-at' : undefined}
              type="datetime-local"
              value={receivedAt}
              disabled={frozen || busy}
          ...
          <Field label="Notes" error={errors.notes} errorId="receipt-error-notes">
            <textarea
              aria-invalid={Boolean(errors.notes)}
              aria-describedby={errors.notes ? 'receipt-error-notes' : undefined}
              value={notes}
              disabled={frozen || busy}
        ...
        {locationError && (
          <p className="mt-3 text-sm text-muted-foreground">
            {locationError} You may continue without a location.
          </p>
        )}
        <fieldset className="mt-5 space-y-3" disabled={frozen || busy}>
          <legend className="font-medium">Purchase Order lines</legend>
              ...
              <div key={line.id} className="rounded-lg border p-4">
                <label className="flex gap-2 font-medium">
                  <input
                    ...
                    disabled={comparePurchaseOrderDecimals(remaining, '0') <= 0}
                    onChange={(e) => updateLine(line.id, { included: e.target.checked })}
                  />
                  {line.line_number}. {line.item_name}
                </label>
                <p className="mt-1 text-sm text-muted-foreground">
                  Ordered {formatPurchaseOrderDecimal(line.ordered_quantity)} · received{' '}
                  {formatPurchaseOrderDecimal(line.received_quantity)} · remaining{' '}
                  {formatPurchaseOrderDecimal(remaining)} {line.ordered_unit}
                </p>
                {value.included && (
                  <div className="mt-3 grid gap-3 sm:grid-cols-4">
                    <Field
                      label={`Quantity (${line.ordered_unit})`}
                      error={errors[`${line.id}.quantity`]}
                      errorId={`receipt-error-${line.id}-quantity`}
                    >
                      <input
                        aria-invalid={Boolean(errors[`${line.id}.quantity`])}
                        aria-describedby={
                          errors[`${line.id}.quantity`]
                            ? `receipt-error-${line.id}-quantity`
                            : undefined
                        }
                        inputMode="decimal"
                    ...
                    <Field
                      label="Lot code"
                      error={errors[`${line.id}.lot`]}
                      errorId={`receipt-error-${line.id}-lot`}
                    >
                      <input
                        aria-invalid={Boolean(errors[`${line.id}.lot`])}
                        aria-describedby={
                          errors[`${line.id}.lot`] ? `receipt-error-${line.id}-lot` : undefined
                        }
                        value={value.lotCode}
                        onChange={(e) => updateLine(line.id, { lotCode: e.target.value })}
                        className="w-full rounded-md border bg-background p-2"
                      />
                    </Field>
                    <Field label="Storage location">
                      <select
                        ...
                        disabled={!warehouseId || Boolean(locationError)}
                        onChange={(e) => updateLine(line.id, { storageLocationId: e.target.value })}
                        className="w-full rounded-md border bg-background p-2"
                      >
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          ...
          <button
            type="button"
            disabled={busy || Boolean(attempt?.uncertain)}
            onClick={onClose}
          ...
          {attempt?.uncertain && attemptBelongsToCurrentPurchaseOrder ? (
            ...
            >
              {busy ? 'Retrying…' : 'Retry same receipt'}
            </button>
          ) : (
            <button
              type="button"
              disabled={busy || frozen || Boolean(optionsError)}
              onClick={submit}
              className="rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground"
            >
