...
export function PurchaseReceiptHistory({
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      summary
        ? listReceiptStorageLocations(summary.warehouse_id, controller.signal).catch(() => [])
        : Promise.resolve([]),
    ])
      .then(([received, locations]) => {
        ...
        setLocationLabels(
          new Map(
            locations.map((location) => [location.id, `${location.name} (${location.code})`]),
          ),
        );
      })
      .catch((caught) => {
      ...
      {loading ? (
        ...
        <div className="mt-4">
          <EmptyState
            ...
            description="Nothing has been received against this Purchase Order."
          />
        </div>
      ) : (
          ...
          {receipts.map((receipt) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span>
                    <span className="font-medium">{receipt.grn}</span>
                    <span className="ml-2 text-sm text-muted-foreground">
                      {timestamp(receipt.received_at)}
                    </span>
                  </span>
                  <span className="text-sm text-muted-foreground">
                    {warehouseLabels.get(receipt.warehouse_id) ?? 'Warehouse unavailable'} ·{' '}
  ...
  return (
    <div className="space-y-4">
      <dl className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Info
          label="Warehouse"
          value={warehouseLabels.get(receipt.warehouse_id) ?? 'Warehouse unavailable'}
        />
        <Info
          label="Supplier delivery reference"
          value={receipt.supplier_delivery_reference ?? '—'}
        />
        <Info
          label="Received by"
          value={receipt.received_by_id === currentUserId ? 'You' : receipt.received_by_id}
        />
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b">
              ...
              <th className="p-2">Price snapshot</th>
            </tr>
          </thead>
          <tbody>
            {receipt.lines.map((line) => {
              const poLine = poLines.get(line.purchase_order_line_id);
              return (
                <tr key={line.id} className="border-b last:border-0">
                  <td className="p-2">
                    {poLine
                      ? `${poLine.line_number}. ${poLine.item_name}`
                      : 'Purchase Order line unavailable'}
                  </td>
                  <td className="p-2">
                    {formatPurchaseOrderDecimal(line.quantity)} {line.ordered_unit}
                  </td>
                  <td className="p-2">{line.lot_code}</td>
                  <td className="p-2">
                    {line.storage_location_id
                      ? (locationLabels.get(line.storage_location_id) ?? 'Location unavailable')
                      : '—'}
                  </td>
                  <td className="p-2">{line.expiry_date ?? '—'}</td>
                  <td className="p-2">
                    {formatPurchaseOrderMoney(line.unit_price, line.currency_code)}
                  </td>
                </tr>
              );
            })}
