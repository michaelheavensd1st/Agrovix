...
## Sprint 5.1 — Inventory Dashboard (PR #6, in review)
Read-only inventory dashboard + workspace organization-context
hardening. **Frontend-only.** No new backend endpoints, schemas, or
migrations.

### Delivered
- `/inventory/dashboard`: summary cards, attention panel, activity
  placeholder, quick actions. Uses only existing Sprint 4 endpoints.
- Organization preservation: `?organization_id=` respected on
  dashboard + workspace; propagated to every quick-action link.
- Lot fan-out 401 → `/login`; 403 → `ForbiddenBanner`; non-auth
  partial → "understated totals" warning.
- Bootstrap 401/403 → dedicated auth handling.
- Dashboard stale-response guard via `activeGenerationRef`.
- Workspace org-switch reset: clears warehouses / items / lots /
  history + form-local state (`key={orgId}` remounts).

### PR #6 review round #3 (2026-02-15 — this commit)
- **Workspace async race guard**: `orgGenerationRef`,
  `lotGenerationRef`, `historyGenerationRef` in
  `/apps/web/app/inventory/page.tsx`. Every organization-scoped
  fetch (reloadOrg / reloadLots / lot history) captures the active
  orgId + selected warehouse + lot at request start; obsolete
  responses (e.g. a slow org-A warehouses fetch that resolves after
  the user has switched to org-B) are dropped and cannot overwrite
  state.
- **Regression tests A / B / C / D**
  (`/apps/web/tests/inventory-workspace-org-context.test.tsx`)
  use deferred promises to reproduce and prove the race is fixed
  for: (A) org-A warehouse + item fan-out arriving late,
  (B) org-A lot fetch arriving late, (C) org-A lot-history fetch
  arriving late, (D) obsolete org-A warehouse / item IDs cannot be
  written from the Receive form after a race.
- **Dashboard race test rewrite**
  (`/apps/web/tests/inventory-dashboard.test.tsx`): replaced
  never-resolving `new Promise(() => {})` stalls with deferred
  promises that fully complete organization A (warehouses + items
  + lots) inside `act(...)` after B has rendered — this actually
  exercises the generation-guard write branch and eliminates all
  `act()` warnings.

### Validation gate (this commit)
- `prettier --check` ✅
- `tsc --noEmit` ✅
- `next lint` ✅ (no warnings)
- `vitest --run` ✅ 53/53 pass, zero `act()` warnings
- `next build` ✅
- backend `pytest` ✅ 213 pass / 31 skipped (live-only + pg-only)

### Commit
`2258d5d fix(inventory-ui): guard workspace against stale organization requests`
— committed locally on `feature/sprint-5-1-inventory-dashboard`.
NOT pushed. NOT PR-updated. User to sync via "Save to Github".

### Explicitly out-of-scope (deferred)
- Dashboard fan-out concurrency limits
- Malformed balance handling
- Reactive same-page URL changes
- Any new backend endpoints, schemas, or migrations
- Sprint 5.2 preflight
- Remaining Sprint 5 screens (Inventory-item CRUD, Warehouse CRUD,
  full transaction forms, supplier management, procurement,
  purchase orders)
